...
public class Game {
    Board board; // Board class has size and char[][] board
    Player[] players;
...
//            crossPattern += "X";
    ...
    public void play() {
        ...
        while(true) {
            ...
            board.board[rn][cn] = players[turn].symbol;//name of object is board and array is the board
            if(this.noOfMoves >= 2 * board.size - 1 && checkIfTheGameIsEnded()) {
                System.out.println(board);
                System.out.println(players[turn].name + " has won !!");
                return;
            }
            turn = (turn + 1) % 2; //1%2=1,2%2=0,3%2=1,4%2=0
            System.out.println(board);
        }
    }
...
//            int arr[]={rn, cn};
            return new int[]{rn, cn}; // rn = idxs[0], cn = idxs[1];
        }
    }
